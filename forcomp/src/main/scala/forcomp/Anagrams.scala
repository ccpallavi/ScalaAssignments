...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
  	case Nil => List(Nil)
  	case (_, 0) :: zeroOccurrences => combinations(zeroOccurrences)
  	case (char, count) :: occurrences => combinations((char, count - 1) :: occurrences) ++ combinations((char, count - 1) :: occurrences).map(occurrence =>
  		occurrence.find(_._1 == char) match {
  			case Some(_) => occurrence.map(_ match {
  				case (char1, count1) => if (char1 == char) (char1, count1 + 1) else (char1, count1)
  			})
  			case None => (char, 1) :: occurrence
  		})
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    x.map(x => y.find(_._1 == x._1) match {
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
	...
	def sentenceAnagramsHelper(occurrences: Occurrences, accumulator: List[Sentence]): List[Sentence] =
		if (occurrences.isEmpty) 
			...
			} yield sentenceAnagramsHelper(subtract(occurrences, elem), accumulator.map(List(wordAnagram) ::: _))).flatten
		}
