...
trait Solver extends GameDef {
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = b.legalNeighbors.map(x => x match {case (block, moves) => (block, moves :: history)}).toStream
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
                       explored: Set[Block]): Stream[(Block, List[Move])] = neighbors filterNot {case (block, moves) => explored contains block}
  ...
  def from(initial: Stream[(Block, List[Move])],
	...
	case (block, moves) #:: rest => {
	  ...
	  more ++ from(rest ++ more, explored + block)
	}
  }
